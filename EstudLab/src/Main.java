...
public class Main {
	...
	public static void main(String[] args) {
		...
		while(quantContas <= 0){
			System.out.println("Quantas contas serao criadas?");
		...
		for(int i=0; i<contas.length; i++) {
			System.out.println("Digite 1 para Conta Corrente ou outro valor para conta Poupanca:");
		...
		for(ContaBancaria cb: contas)
			if(cb instanceof ContaPoupanca)
				System.out.println("Taxa de rendimento da poupanca:"+((ContaPoupanca) cb).getTaxaRendimento());
			else if(cb instanceof ContaCorrente)
				System.out.println("Quantidade de transacoers da conta corrente"  +cb.getNumero()+" : "+(((ContaCorrente)cb).getQuantTransacoes()));
